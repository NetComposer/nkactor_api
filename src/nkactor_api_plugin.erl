...
%% @doc
plugin_config(_SrvId, Config, #{class:=?PACKAGE_NKACTOR_API}) ->
    Syntax = #{
        actor_service => atom,
        debug => boolean,
        '__mandatory' => [actor_service]
    },
...
plugin_cache(_SrvId, Config, _Service) ->
    Cache = #{
        actor_service => maps:get(actor_service, Config, false),
