...
%% @doc NkACTOR API Application Module
-module(nkactor_api_app).
...
start(_Type, _Args) ->
    ...
    case nklib_config:load_env(?APP, Syntax) of
        {ok, _} ->
            {ok, Pid} = nkactor_api_sup:start_link(),
            {ok, Vsn} = application:get_key(nkactor, vsn),
            ok = nkserver_util:register_package_class(?PACKAGE_NKACTOR_API, nkactor_api),
            lager:info("NkACTOR API v~s has started.", [Vsn]),
            {ok, Pid};
        {error, Error} ->
